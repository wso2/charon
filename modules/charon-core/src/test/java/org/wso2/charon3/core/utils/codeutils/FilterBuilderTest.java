...
/**
 * author Pascal Knueppel <br> created at: 22.03.2019 - 15:33 <br>
 * <br>
 * .
 */
public class FilterBuilderTest {
    ...
    /**
     * shows that the filter-builder does also work if the fieldnames are added directly.
     */
    @Test
    public void testFilterBuilderWithStrings() {
        final String randomValue = UUID.randomUUID().toString();
        String filter = FilterBuilder.builder().and("emails.display", EQ, randomValue).build();
        Assertions.assertEquals("emails.display eq \"" + randomValue + "\"", filter);

        filter = FilterBuilder.builder().and("emails.type", EQ, randomValue).build();
        Assertions.assertEquals("emails.type eq \"" + randomValue + "\"", filter);

        filter = FilterBuilder.builder().and("emails.type", CO, randomValue).build();
        Assertions.assertEquals("emails.type co \"" + randomValue + "\"", filter);

        filter = FilterBuilder.builder().and("phoneNumbers.display", CO, randomValue).build();
        Assertions.assertEquals("phoneNumbers.display co \"" + randomValue + "\"", filter);

        filter = FilterBuilder.builder().and("manager", CO, randomValue).build();
        Assertions.assertEquals("manager co \"" + randomValue + "\"", filter);

        filter = FilterBuilder.builder().and("manager", CO, randomValue)
            .and("userName", SW, randomValue).build();
        Assertions.assertEquals("manager co \"" + randomValue + "\" and userName sw \""
            + randomValue + "\"", filter);

        filter = FilterBuilder.builder().and("emails.value", EQ, randomValue)
            .and("userName", SW, randomValue)
            .leftParenthesis()
            .or("nickName", EQ, randomValue)
            .or("name.givenName", EQ, randomValue)
            .rightParenthesis()
            .and("name.familyName", EQ, randomValue).build();
        Assertions.assertEquals(
            "emails.value eq \"" + randomValue + "\" and "
                + "userName sw \"" + randomValue + "\" or "
                + "(nickName eq \"" + randomValue + "\" or "
