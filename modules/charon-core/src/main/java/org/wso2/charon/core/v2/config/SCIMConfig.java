    ...
    public SCIMConsumer getConsumerProcessed(String consumerId) {
        if (consumersMap != null && consumersMap.containsKey(consumerId)) {
            ...
            if (scimConsumer.isIncludeAll()) {
                ...
                for (Map.Entry<String, SCIMProvider> providerId : providersMap.entrySet()) {
                    if (!providers.containsKey(providerId.getKey())) {
                        providers.put(providerId.getKey(), providersMap.get(providerId.getKey()));
                    }
                }
            }
